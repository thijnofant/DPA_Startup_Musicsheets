...
{
    public class AddCleffHandler : ChainHandler
    {
        public AddCleffHandler(Context con, CommandRegister reg, ChainHandler nextHandler = null) : base(con, reg, nextHandler)
        {
        }
        public override bool Handle(List<Key> keys)
        {
            if (CanHandle(keys))
            {
                ...
                this.commandRegister.ExcecuteCommand("AddCleff");
        ...
        {
            return CompareKeyLists(keys, KeyboardConfig.Instance.config["AddCleff"]);
        }
    }
}
