...
{
    public class AddTempoHandler : ChainHandler
    {
        public AddTempoHandler(Context con, CommandRegister reg, ChainHandler nextHandler = null) : base(con, reg, nextHandler)
        {
        }
        public override bool Handle(List<Key> keys)
        {
            if (CanHandle(keys))
            {
                ...
                this.commandRegister.ExcecuteCommand("AddTempo");
        ...
        {
            return CompareKeyLists(keys, KeyboardConfig.Instance.config["AddTempo"]);
        }
    }
}
