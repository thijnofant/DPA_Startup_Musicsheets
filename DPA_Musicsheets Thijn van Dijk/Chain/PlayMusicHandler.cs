...
{
    public class PlayMusicHandler : ChainHandler
    {
        public PlayMusicHandler(Context con, CommandRegister reg, ChainHandler nextHandler = null) : base(con, reg, nextHandler)
        {
        }
        public override bool Handle(List<Key> keys)
        {
            if (CanHandle(keys))
            {
                Debug.WriteLine("Play music Chain handler handling");
                this.commandRegister.ExcecuteCommand("PlayMusic");
        ...
        {
            return CompareKeyLists(keys, KeyboardConfig.Instance.config["PlayMusic"]);
        }
    }
}
